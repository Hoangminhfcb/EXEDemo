...
import { FaArrowLeft, FaChartLine, FaShoppingCart, FaStar, FaEye, FaEdit, FaPlus } from "react-icons/fa"
import { LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, BarChart, Bar } from "recharts"

export default function BakeryDashboard({ params }: { params: { id: string } }) {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="bg-white shadow-sm">
        <div className="container mx-auto px-4 py-4 flex justify-between items-center">
          <div className="flex items-center">
            ...
            <h1 className="text-2xl font-bold text-gray-900">Dashboard - Sweet Dreams Bakery</h1>
          </div>
      ...
      <div className="container mx-auto px-4 py-6">
          ...
          <div className="bg-white rounded-lg shadow-sm p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-gray-600">{stats.pendingOrders} đơn chờ xử lý</p>
              </div>
              ...
              <div>
                ...
                <div className="flex items-center">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <FaStar
                      key={star}
                      className={`w-4 h-4 ${star <= Math.floor(stats.averageRating) ? "text-yellow-400" : "text-gray-300"}`}
                    />
                  ))}
                </div>
              </div>
          ...
          <div className="bg-white rounded-lg shadow-sm p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-gray-600">Khách mới: {stats.newCustomers}</p>
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
          ...
          <div className="bg-white rounded-lg shadow-sm p-6">
            ...
            <ResponsiveContainer width="100%" height={300}>
              <LineChart data={chartData}>
                ...
                <XAxis dataKey="name" />
                <YAxis />
                <Tooltip formatter={(value) => [formatPrice(Number(value)), "Doanh thu"]} />
          ...
          {/* Orders Chart */}
          <div className="bg-white rounded-lg shadow-sm p-6">
            <h3 className="text-lg font-semibold mb-4">Số đơn hàng 7 ngày qua</h3>
            <ResponsiveContainer width="100%" height={300}>
        ...
        <div className="bg-white rounded-lg shadow-sm p-6">
          <div className="flex justify-between items-center mb-6">
            ...
            <Link href={`/bakery/${bakeryId}/orders`} className="text-pink-600 hover:text-pink-700 text-sm font-medium">
              Xem tất cả
            </Link>
          </div>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {recentOrders.map((order) => (
                  <tr key={order.id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{formatPrice(order.amount)}</td>
