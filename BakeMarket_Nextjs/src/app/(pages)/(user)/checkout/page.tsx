...
import { calculateOrderTotals, groupCartItemsByBakery, formatPrice, validateCustomerInfo } from "@/utils/checkout-utils"
import { getCart, updateCartItemQuantity, removeCartItem, type CartItem } from "@/utils/cartStorage"
...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    ...
    if (items.length === 0) {
      showToast("Giỏ hàng trống", "Vui lòng thêm sản phẩm vào giỏ hàng trước khi thanh toán", "destructive")
      router.push("/cart")
    }
  }, [router])
  ...
  const handleSubmitOrder = async () => {
    ...
    if (!validation.isValid) {
      showToast("Thông tin không hợp lệ", validation.errors.join(", "), "destructive")
      return
    }
    ...
    try {
      ...
      showToast(
        "Lỗi đặt hàng",
        error instanceof Error ? error.message : "Có lỗi xảy ra khi đặt hàng. Vui lòng thử lại.",
        "destructive",
      )
    } finally {
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="flex items-center mb-6">
          <Link href="/cart" className="flex items-center text-pink-600 hover:text-pink-700">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200">
              <div className="px-6 py-4 border-b border-gray-200">
                <h3 className="text-lg font-semibold">Đơn hàng của bạn ({orderTotals.itemCount} sản phẩm)</h3>
              </div>
              <div className="p-6 space-y-6">
                <div className="space-y-6">
                  {Object.values(groupedItems).map((group) => (
                    <div key={group.bakeryId} className="border border-gray-200 rounded-lg p-4">
                      <h3 className="font-medium text-gray-900 mb-4">{group.bakeryName}</h3>
              ...
              <div className="p-6 space-y-4">
                <div className="space-y-2">
                  ...
                  <div className="flex justify-between text-sm">
                    ...
                    <span className={orderTotals.shippingFee === 0 ? "text-green-600" : ""}>
                      {orderTotals.shippingFee === 0 ? "Miễn phí" : formatPrice(orderTotals.shippingFee)}
                    </span>
                  </div>
                  {orderTotals.subtotal < 5000000 && orderTotals.shippingFee > 0 && (
                    <div className="text-xs text-gray-500">
                      Mua thêm {formatPrice(5000000 - orderTotals.subtotal)} để được miễn phí giao hàng
                    </div>
                  )}
                  <div className="flex justify-between font-semibold text-lg pt-2 border-t border-gray-200">
                    <span>Tổng cộng:</span>
                    <span className="text-pink-600">{formatPrice(orderTotals.total)}</span>
                  </div>
                </div>
